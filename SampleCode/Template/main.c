...
void UART0_IRQHandler(void)
{
    ...
    if(UART_GET_INT_FLAG(UART0, UART_ISR_RDA_IS_Msk | UART_ISR_RTO_IS_Msk))     /* UART receive data available flag */
    {
...
void UART0_Init(void)
{
    ...
    UART_EnableInt(UART0, UART_IER_RDA_IE_Msk | UART_IER_RTO_IE_Msk);
...
void SYS_Init(void)
{
    ...
    CLK_WaitClockReady(CLK_CLKSTATUS_HXT_STB_Msk|CLK_CLKSTATUS_HIRC_STB_Msk);

    /* Enable HIRC clock  */
    CLK_EnableXtalRC(CLK_PWRCTL_LXT_EN_Msk|CLK_PWRCTL_LIRC_EN_Msk);

    /* Wait for HIRC clock ready */
    CLK_WaitClockReady(CLK_CLKSTATUS_LXT_STB_Msk|CLK_CLKSTATUS_LIRC_STB_Msk);	
...
int main()
{
    ...
    CLK_EnableCKO(CLK_CLKSEL2_FRQDIV_S_HIRC, 0);		// 12 / 2 ^ (0+1) = 12 / 2 = 6M
